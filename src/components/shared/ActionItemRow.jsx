...
export default function ActionItemRow({ item, onToggleStatus, onToggleStar, onPress }) {
  ...
  return (
    ...
    >
      {/* Status toggle */}
      <button onClick={handleStatusTap} className="mt-0.5 flex-shrink-0">
        <StatusIcon
          size={22}
          className={
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className={`text-sm font-medium truncate ${isComplete ? 'line-through text-gray-400' : 'text-gray-900'}`}>
            {item.title}
          </span>
          {item.starred && (
      ...
      {/* Star toggle */}
      <button onClick={handleStarTap} className="mt-0.5 flex-shrink-0">
        <Star
          size={18}
